...
void settings() {
  size((finalW << 1) + finalH, finalH, P2D);
}
...
void draw() {
  ...
  hough = hough(sobel, 6, linesImg, lines);  
...
PImage hough(PImage edgeImg, int nLines, PGraphics linesImg, ArrayList<PVector> vectors) {
  ...
  ArrayList<PVector> newVectors = new ArrayList<PVector>();
  linesImg.beginDraw();
  ...
  getIntersections(newVectors, linesImg);
  return houghImg;
}
